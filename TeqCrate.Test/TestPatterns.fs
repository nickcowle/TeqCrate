...
module TestPatterns =
            ...
            let t1, t2, t3 =
                c.Apply
                    { new TripleTeqEvaluator<_, _> with
                        member __.Eval(teq: Teq<int * string * bool, 'a * 'b * 'c>) = typeof<'a>, typeof<'b>, typeof<'c> }
    ...
    type TestRecord = { Foo: string; Bar: int; Baz: string }
                ...
                c.Apply
                    { new RecordConvEvaluator<_, _> with
                        member __.Eval<'a> names ts (conv: Conv<TestPrivateRecord, 'a HList>) =
                            ...
                            TypeList.toTypes ts = [ typeof<string>
                                                    typeof<int>
                                                    typeof<string> ] }
            | _ -> false
    ...
    let ``Record active pattern recognises a public record whose fields are private`` () =
        ...
        let expected =
            Some [ "InternallyPrivateBaz", "world"
                   "InternallyPrivateFoo", "hello" ]
